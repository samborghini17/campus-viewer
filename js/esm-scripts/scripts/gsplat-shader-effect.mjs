 ...
 */ class GsplatShaderEffect extends Script {
    ...
    removeShaders() {
        ...
        this.materialsApplied.forEach((material)=>{
            // FIX: Verwende den neuen Chunk-Namen
            material.getShaderChunks(shaderLanguage).delete('gsplatModifyVS');
            // Sicherheitshalber auch den alten entfernen, falls noch Reste da sind
     ...
     * @type {number}
     */ _define_property(this, "effectTime", 0), /**
     * Set of materials with applied shader
     * @type {Set<import('playcanvas').Material>}
     */ _define_property(this, "materialsApplied", new Set());
    }
}
_define_property(GsplatShaderEffect, "scriptName", 'gsplatShaderEffect');
